...
describe("stringAssist", () => {
  ...
  it("multi", () => {
    const result = StringUtil.start(20000000000)
      .multi(`20730`)
      .unShiftedBy(new Calculator(18))
      .end();
  ...
  it("remainDecimal_", () => {
    const result = StringUtil.start(new Calculator(`10.12`))
      .remainDecimal(4, RoundingMode.ROUND_UP)
      .end();
    assert.strictEqual(result, `10.12`);

    const result1 = StringUtil.start(`10.12`)
      .remainDecimal(4, RoundingMode.ROUND_UP, true)
      .end();
    assert.strictEqual(result1, `10.1200`);

    const result2 = StringUtil.start(`10.12`)
      .remainDecimal(1, RoundingMode.ROUND_UP)
      .end();
    assert.strictEqual(result2, `10.2`);

    const result3 = StringUtil.start(`10.12`)
      .remainDecimal(0, RoundingMode.ROUND_UP)
      .end();
    assert.strictEqual(result3, `11`);

    const result4 = StringUtil.start(`10`)
      .remainDecimal(4, RoundingMode.ROUND_UP)
      .end();
  ...
  it("toArray_1", () => {
    const result: string[] = StringUtil.toArray(
      "7cce3caf92eb2005ad8a6dd02625d1d8",
      2
    );
    // console.error(result)
  ...
  it("toArray_2", () => {
    ...
    assert.strictEqual(
      ...
      '["7cce3caf","92eb2005","ad8a6dd0","2625d1d8"]'
    );
  });
  ...
  it("binToDecimalString_", () => {
    const result: string = StringUtil.start(
      "0b11001100111001100000110010001000001011010010101100110101011"
    )
      .toDecimalString()
      .end();
  ...
  it("decimalToBinString", () => {
    const result: string = StringUtil.start("461390693981051307")
      .toBinString()
      .end();
    // console.error(result)
    assert.strictEqual(
  ...
  it("numberStrToNumber", () => {
    const result: number = StringUtil.numberStrToNumber("190");
    // console.error(result)
    assert.strictEqual(result, 190);
  });

  it("hexStrToBase64", () => {
    const result: string = StringUtil.hexStrToBase64(
      "0c68affbcf01a49540e590bcf88afa4dc630eb5ce0e13a5b067c918f0e1c9af4"
    );
    // console.error(result)
  ...
  it("strToBase64", () => {
    const result: string = StringUtil.strToBase64(
      "0c68affbcf01a49540e590bcf88afa4dc630eb5ce0e13a5b067c918f0e1c9af4"
    );
    // console.error(result)
    assert.strictEqual(
  ...
  it("base64ToHexStr", () => {
    const result: string = StringUtil.base64ToHexStr(
      "DGiv+88BpJVA5ZC8+Ir6TcYw61zg4TpbBnyRjw4cmvQ=",
      false
    );
    // console.error(result)
    assert.strictEqual(
  ...
  it("base64ToStr", () => {
    const result: string = StringUtil.base64ToStr(
      "MGM2OGFmZmJjZjAxYTQ5NTQwZTU5MGJjZjg4YWZhNGRjNjMwZWI1Y2UwZTEzYTViMDY3YzkxOGYwZTFjOWFmNA=="
    );
    // console.error(result)
    assert.strictEqual(
  ...
  it("toUtf8Buffer_", () => {
    ...
    assert.strictEqual(
      JSON.stringify(result),
      `{"type":"Buffer","data":[49,115,104,102,103,100,106,101,119,97,114,116,97]}`
    );
  });
  ...
  it("toUtf8Uint8Array_", () => {
    ...
    assert.strictEqual(
      JSON.stringify(result),
      `{"0":49,"1":115,"2":104,"3":102,"4":103,"5":100,"6":106,"7":101,"8":119,"9":97,"10":114,"11":116,"12":97}`
    );
  });
